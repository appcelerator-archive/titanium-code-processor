...
function DateType(pattern, flags, className) {
	
	Base.ObjectType.call(this, className || "Date");
	this.objectPrototype = new DatePrototypeType();
}
util.inherits(DateType, Base.ObjectType);
...
MathPowFunc.prototype.call = function call(thisVal, args) {
	...
	if (!x || !y) {
		return new Base.NumberType(NaN);
	} else if (Base.type(x) === "Unknown" || Base.type(y) === "Unknown") {
		return new Base.UnknownType();
	} else {

...
EvalFunction.prototype.call = function call(thisVal, args) {
	...
	// Step 4
	result = RuleProcessor.processBlock(prog[1]);
...
FunctionConstructor.prototype.construct = function call(args) {
	...
	// Step 10
	return new Base.FunctionType(p, body, Runtime.getModuleContext().lexicalEnvironment, 
	...
	if (args.length === 1) {
		...
		if (Base.type(len) === "Number") {
			if (len.value === Base.toUint32(len).value) {
				...
				throw new Exceptions.RangeError('Invalid length ' + len.value);
			}
		} else {
...
JSONParseFunc.prototype.call = function call(thisVal, args) {
	...
	try {
		...
		throw new Exceptions.SyntaxError(e.message);
	}

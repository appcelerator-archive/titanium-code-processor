...
exports.createGlobalContext = function createGlobalContext(ast, strict) {
	...
	var globalObject = new Base.ObjectType(),
		env = NewObjectEnvironment(globalObject, Runtime.globalContext.lexicalEnvironment),
		configurableBindings = false,
		executionContext = new ExecutionContext(
			NewObjectEnvironment(globalObject, Runtime.globalContext.variableEnvironment),
			env,
			globalObject,
			strict
		),
		len, i,
		functions = findFunctionDeclarations(ast),
		variables = findVariableDeclarations(ast);
	
	// Find all of the function declarations and bind them
	for (i = 0, len = functions.length; i < len; i++) {
		var fn = functions[i].functionName,
			fo = new Functions.FunctionType(functions[i].functionBody, functions[i].formalParameterList),
...
/**
 * The global context, including the global object. Initialized via a call to {@link module:Context.initVM}.
 * 
 * @type module:Context.EnvironmentContext
 */

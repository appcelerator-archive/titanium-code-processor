...
function findDeclarations(ast) {
	...
	while(nodeStack.length) {
		...
		if (name === "defun") {
			functions.push(node[1]);
		} else if (name === "var") {
			for(i = 0, len = node[1].length; i < len; i++) {
				variables.push(node[1][i][0]);
			}
		} else {
...
exports.createGlobalContext = function createGlobalContext(ast, strict) {
	...
	var globalObject = new Base.ObjectType(),
		...
		executionContext = new ExecutionContext(
			...
			NewObjectEnvironment(globalObject, Runtime.globalContext.lexicalEnvironment),
			globalObject,
			strict
		),
		len, i,
	...
	variables = result.functions;
	
	// Find all of the function declarations and bind them
	for (i = 0, len = functions.length; i < len; i++) {
		var fn = functions[i].functionName,
			fo = new Base.FunctionType(functions[i].functionBody, functions[i].formalParameterList),

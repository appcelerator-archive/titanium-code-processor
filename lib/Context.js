 ...
 *
 * @method
 * @param {module:Context.FunctionType} func ECMA-262 Spec: <em>the function object whose code is to be evaluated</em>
 ...
 * @param {module:Context.LexicalEnvironment} env ECMA-262 Spec: <em>the variable environment for the function code</em>
		...
		if (indx < names.length) {
			...
			if (!strict && !(name in mappedNames)) {
				...
				map.defineOwnProperty(indx + "", {
					get: new FunctionType([], AST.parseString("return " + name + ";"), env, true),
	...
	if (mappedNames.length) {
		...
		obj.get = function get(p) {
			...
			if (isMapped) {
				...
				v = Base.ObjectType.prototype.get.call(obj, p);
				if (p === "callee" && Base.type(v) === "Function" && v.strict) {
					throw new Exceptions.TypeError();
				}
				return v;
			}
		...
		obj.getOwnProperty = function getOwnProperty(p) {
			var desc = Base.ObjectType.prototype.getOwnProperty.call(obj, p),
				map,
				isMapped;
		...
		obj.defineOwnProperty = function defineOwnProperty(p, desc, throwFlag) {
			...
			if (isMapped) {
				if (Base.isAccessorDescriptor(desc)) {
					map["delete"](p, false);
				} else {
					if (desc.value) {
		...
		obj["delete"] = function (p, throwFlag) {
			var isMapped = map.getOwnProperty(p),
				result = Base.ObjectType.prototype["delete"].call(obj, p, throwFlag);
			if (result && isMapped) {
				map["delete"](p, false);
			}
			return result;

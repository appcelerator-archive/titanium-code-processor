...
function generateAnnotatedHTML(code, codeStyles, header, headerStyles) {
	function generate(data, styles, showLineNumbers) {
		var i, len, j,
			...
			jumps,
			startTag, endTag,
			projectDir = path.dirname(Runtime.getEntryPointFile());
		...
		for (i = styles.length - 1; i >= 0; i--) {
			...
			if (jumps) {
				...
				for (j in jumps) {
					startTag += '<li><a href="' + path.relative(projectDir, jumps[j].filename) + '.html#' + jumps[j].line + '">' +
						path.relative(projectDir, jumps[j].filename) + ':' + jumps[j].line + '</a></li>';
				}
				startTag += '</ul></div></div>';
			}
			startTag += '<span style="' +

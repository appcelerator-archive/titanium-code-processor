...
function generateAnnotatedHTML(code, codeStyles, header, headerStyles, fileLocation) {
	function generate(data, styles, showLineNumbers) {
		var i, len, j,
			style,
			start,
			end,
			...
			fileDir = path.dirname(fileLocation);
		...
		for (i = styles.length - 1; i >= 0; i--) {
			...
			startTag = '';
			if (style.jumpSources) {
				startTag += '<span class="link"><div class="menu"><div class="menuInner">Jump Sources<ul>';
				for (j in style.jumpSources) {
					startTag += '<li><a href="' + path.relative(fileDir, style.jumpSources[j].filename) + '.html#' +
						style.jumpSources[j].line + '">' +
						path.relative(baseDir, style.jumpSources[j].filename) + ':' +
						style.jumpSources[j].line + '</a></li>';
				}
				startTag += '</ul></div></div>&#x21D8;</span>';
			}
			endTag = '</span>';
			if (style.jumpDestinations) {
				endTag += '<span class="link"><div class="menu"><div class="menuInner">Jump Destinations<ul>';
				for (j in style.jumpDestinations) {
					endTag += '<li><a href="' + path.relative(fileDir, style.jumpDestinations[j].filename) + '.html#' +
						style.jumpDestinations[j].line + '">' +
						path.relative(baseDir, style.jumpDestinations[j].filename) + ':' +
						style.jumpDestinations[j].line + '</a></li>';
				}
				endTag += '</ul></div></div>&#x21D7;</span>';
			}
			startTag += '<span style="' +
		...
		if (showLineNumbers) {
			...
			for (i = 0, len = data.length; i < len; i++) {
				data[i] = '<a name="' + (i + 1) + '">' + ('000' + (i + 1)).slice(-data.length.toString().length) + ':</a> ' + data[i];
			}
			data = data.join('\n');
		}
		return data;
	...
	return '<!DOCTYPE html>\n<html>\n<head>\n<style type="text/css">\n' +
		...
		'<pre>\n' + generate(code, codeStyles, true) + '</pre>\n</body></html>';
}
...
function generateAnnotations(ast, rules) {
	...
	ast.walk(new uglify.TreeWalker(function (node, descend) {
		...
		function generateColor(colorList) {
			...
			for (i = 1, len = colorList.length; i < len; i++) {
		...
		if (previousStyle && node.end) {
			styles.push({
				...
				jumpDestinations: node._jumpDestinations
			});
		}

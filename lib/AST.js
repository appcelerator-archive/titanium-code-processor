 ...
 *
 * @method
 * @param {module:AST.node} ast The AST to minify
 * @returns {{@link module:AST.node}} The minified AST
 */
exports.minify = minify;
function minify(ast) {
	return uglify.uglify.ast_mangle(uglify.uglify.ast_squeeze(ast));
}
...
function generateAnnotatedHTML(serializedCode, styles) {
	...
	for(; i >= 0; i--) {
		style = styles[i];
		start = style.start;
		end = styles[i + 1] ? styles[i + 1].start : serializedCode.length;
		serializedCode = serializedCode.slice(0, start) +
			'<span style="' +
			...
			'">' +
			serializedCode.slice(start, end)
				.replace('"', '&quot;')
				.replace('\'', '&apos;')
				.replace('&', '&amp;')
				.replace('<', ' &lt;')
				.replace('>', '&gt;') +
			'</span>' +
			serializedCode.slice(end);
	}
	return '<!DOCTYPE html><html><body><pre>\n' + serializedCode + '</pre></body></html>';
}
...
function serialize(ast, rules) {
	...
	function walkNode(node) {
		...
		styles.push({
			...
			italic: !!italic,
			bold: !!bold,
			fontColor: generateColor([0, 0, 0], fontColors),
			backgroundColor: generateColor([1, 1, 1], backgroundColors)
		});

...
uglify.AST_Node.prototype._postProcess = function(returnValue) {
	returnValue = Array.isArray(returnValue) ? returnValue[1] : returnValue;
	this._unknown = !!returnValue && returnValue.type === 'Unknown';
	...
	function generate(data, styles, showLineNumbers) {
		...
		if (showLineNumbers) {
			...
			for (i = 0, len = data.length; i < len; i++) {
				data[i] = ('000' + (i + 1)).slice(-data.length.toString().length) + ': ' + data[i];
			}
			data = data.join('\n');
		}
		return data;

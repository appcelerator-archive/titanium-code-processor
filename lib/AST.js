...
function generateAnnotatedHTML(code, codeStyles, header, headerStyles, fileLocation) {
	function generate(data, styles, showLineNumbers) {
		...
		for (i = styles.length - 1; i >= 0; i--) {
			...
			startTag = '';
			if (jumps) {
				startTag += '<span class="link"><div class="menu"><div class="menuInner"><ul>';
				for (j in jumps) {
					startTag += '<li><a href="' + path.relative(fileDir, jumps[j].filename) + '.html#' + jumps[j].line + '">' +
						path.relative(baseDir, jumps[j].filename) + ':' + jumps[j].line + '</a></li>';
				}
				startTag += '</ul></div></div>';
			}
			startTag += '<span style="' +
				...
				'">';
			endTag = '</span>' + (style.endJumps ? '</span>' : '');

 ...
 * 
 * AST: [node-info, identifier <string>, formalParameterList <array[string]>, functionBody <array[ast]>]
 * 
 * @private
 */
...
function processRule(ast) {
	...
	RuleProcessor.logRule('defun', ast[1]);
	...
	if (strict) {
		if (identifier === 'eval' || identifier === 'arguments') {
			Base.throwNativeException('SyntaxError', identifier + ' is not a valid identifier name');
		}
		for (i = 0, len = formalParameterList.length; i < len; i++) {
			if (formalParameterList[i] === 'eval' || formalParameterList[i] === 'arguments') {
				Base.throwNativeException('SyntaxError', formalParameterList[i] + ' is not a valid identifier name');
			}
		}
	}

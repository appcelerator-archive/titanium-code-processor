		...
		if (++Runtime.recursionCount === Runtime.options.maxRecursionLimit) {
			...
			if (Base.type(func) === 'Unknown') {
				...
				if (func.className !== 'Function' || !Base.isCallable(func)) {
					Base.throwNativeException('TypeError', Base.getReferencedName(ref) + ' is not callable');
				}
				...
				// Check if this is a non-local call
				localFunction = func._isLocal();
				
				// Process the arguments
				for (; i < len; i++) {
					argValue = Base.getValue(RuleProcessor.processRule(ast[2][i]));
					if (!localFunction && Base.type(argValue) === 'Object') {
						argValue._leaked = true;
						console.log('Argument being called was leaked');
					}
					args.push(argValue);
				}

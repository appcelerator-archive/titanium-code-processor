...
AST.registerRuleProcessor('AST_New', function processRule() {
	...
	constructor = Base.getValue(RuleProcessor.processRule(this.expression));
	...
	if (Base.type(constructor) === 'Unknown') {
		...
		result = Base.UnknownType();
	} else {

		// Process the arguments
		for (i = 0, len = this.args.length; i < len; i++) {
			args.push(Base.getValue(RuleProcessor.processRule(this.args[i])));
		}

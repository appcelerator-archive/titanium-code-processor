...
function processRule(ast) {
	...
	if (Base.type(leftReference) === 'Unknown') {
		...
		if (Base.type(leftReference) === 'Reference' && Base.isStrictReference(leftReference) && 
			...
			Base.throwException('ReferenceError', 'Invalid reference ' + Base.getReferencedName(leftReference));
		}
				...
				switch (operator) {
					...
					case 'isntanceof':
						if (Base.type(rightValue) !== 'Object' || !rightValue.hasInstance) {
							Base.throwException('TypeError', Base.getReferencedName(rightReference) + 
					...
					case 'in': 
						if (Base.type(rightValue) !== 'Object') {
							Base.throwException('TypeError', Base.getReferencedName(rightReference) + ' is not an object');
						}

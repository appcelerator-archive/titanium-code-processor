...
function processRule(ast) {
	...
	RuleProcessor.logRule('assign', (typeof ast[1] === 'string' ? ast[1] + '=': ''));
	...
	if (Base.type(leftReference) === 'Unknown') {
		...
		if (Base.type(leftReference) === 'Reference' && Base.isStrictReference(leftReference) && 
			...
			Base.throwNativeException('ReferenceError', 'Invalid reference ' + Base.getReferencedName(leftReference));
		}
		...
		if (typeof operator === 'string') {
			...
			if (Base.type(Base.toPrimitive(leftValue)) === 'String' || Base.type(Base.toPrimitive(rightValue)) === 'String') {
				...
				switch (operator) {
					...
					case 'isntanceof':
						if (Base.type(rightValue) !== 'Object' || !rightValue.hasInstance) {
							Base.throwNativeException('TypeError', Base.getReferencedName(rightReference) + 
					...
					case 'in': 
						if (Base.type(rightValue) !== 'Object') {
							Base.throwNativeException('TypeError', Base.getReferencedName(rightReference) + ' is not an object');
						}

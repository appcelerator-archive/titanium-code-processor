...
AST.registerRuleProcessor('AST_Switch', function processRule() {
	...
	input = Base.getValue(RuleProcessor.processRule(this.expression));
	...
	if (Base.type(input) === 'Unknown') {
		...
		for (i = 0, len = this.body.length; i < len; i++) {
			this.body[i].expression && RuleProcessor.processRule(this.body[i].expression);
		...
		if (defaultClause) {
			...
			while (true) {
				...
				while (true) {
					...
					clauseSelector = Base.getValue(RuleProcessor.processRule(c.expression));
					if (!found) {
					...
					if (found && c.body) {
						for (i = 0, len = c.body.length; i < len; i++) {
							r = RuleProcessor.processRule(c.body[i]);
							if (r[1]) {
								v = r[1];
							}
							if (r[0] !== 'normal') {
				...
				if (!foundInB && defaultClause.body) {
					for (i = 0, len = defaultClause.body.length; i < len; i++) {
						r = RuleProcessor.processRule(defaultClause.body[i]);
						if (r[1]) {
							v = r[1];
						}
						if (r[0] !== 'normal') {

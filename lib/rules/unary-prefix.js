	...
	if (Base.type(expr) === 'Reference' && !Base.isUnresolvableReference(expr) && Base.type(Base.getValue(expr)) === 'Unknown') {
		...
		if (Base.type(expr) !== 'Reference') {
			...
			if (Base.isStrictReference(expr)) {
				Base.throwException('ReferenceError', base.getReferencedName(expr) + ' is not resolvable');
			}
			result = true;
			...
			if (Base.isStrictReference(expr)) {
				Base.throwException('SyntaxError', 'Invalid reference ' + base.getReferencedName(expr));
			}
		...
		if (Base.type(expr) === 'Reference' && Base.isStrictReference(expr) && 
			...
			Base.throwException('ReferenceError', Base.getReferencedName(expr) + ' is not a valid identifier name');
		}

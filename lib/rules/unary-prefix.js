	...
	if (Base.type(Base.getValue(expr)) === "Unknown") {
		...
		if (Base.type(expr) !== "Reference") {
			...
			if (Base.isStrictReference(expr)) {
				throw new Exceptions.ReferenceError(base.getReferencedName(expr) + ' is not resolvable');
			}
			result = true;
			...
			if (Base.isStrictReference(expr)) {
				throw new Exceptions.SyntaxError('Invalid reference ' + base.getReferencedName(expr));
			}
		...
		if (Runtime.ambiguousCode) {
			...
			if (Base.type(expr) === "Reference" && Base.isStrictReference(expr) && 
				...
				throw new Exceptions.ReferenceError(Base.getReferencedName(expr) + ' is not a valid identifier name');
			}

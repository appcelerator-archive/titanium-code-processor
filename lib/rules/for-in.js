...
exports.processRule = function processRule(ast) {
	...
	if (experValueType === "Unknown" || Runtime.ambiguousCode || !Runtime.options.evaluateLoops) {
		...
		for (p in obj._properties) {
			...
			if (++loopIterations === Runtime.options.maxLoopIterations) {
				...
				Messaging.fireEvent("maxIterationsExceeded", eventData);
				eventData.description = "Max iterations exceeded";
				Messaging.reportWarning(eventData);
				Runtime.ambiguousCode++;

		...
		switch (operator) {
			...
			case "isntanceof":
				if (Base.type(rightValue) !== "Object" || !rightValue.hasInstance) {
					throw new Exceptions.TypeError('Expression is not an object or does not have a hasInstance method');
				}
				result.value = rightValue.hasInstance(leftValue);
				break;
			case "in": 
				if (Base.type(rightValue) !== "Object") {
					throw new Exceptions.TypeError('Expression is not an object');
				}

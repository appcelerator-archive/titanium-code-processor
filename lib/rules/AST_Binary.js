...
AST.registerRuleProcessor('AST_Binary', function processRule() {
	...
	leftValue = Base.getValue(RuleProcessor.processRule(this.left));

	if (Base.type(leftValue) === 'Unknown') {
		result = new Base.UnknownType();
	} else if (~['*', '/', '%', '-', '<<', '>>', '>>>', '&', '|', '^'].indexOf(operator)) {
		result = new Base.NumberType();
		rightValue = Base.getValue(RuleProcessor.processRule(this.right));
		...
		if (!Base.toBoolean(leftValue).value) {
			...
			result = rightValue = Base.getValue(RuleProcessor.processRule(this.right));
		}
	} else if (operator === '||') {
		if (Base.toBoolean(leftValue).value) {
			result = leftValue;

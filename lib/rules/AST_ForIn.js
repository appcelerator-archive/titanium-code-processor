	...
	if (experValueType === 'Unknown' || !Runtime.options.evaluateLoops) {
		...
		outerLoop: while (obj) {
			...
			for (i = 0, len = propNames.length; i < len; i++) {
				...
				} else if (loopIterations == 1) { // The loop did not run, so mark as skipped
					// TODO: need to set unknown value for prop iterator
					Base.processInSkippedMode(this.init.processRule.bind(this.init), this.body.processRule.bind(this.body));
				}
			}

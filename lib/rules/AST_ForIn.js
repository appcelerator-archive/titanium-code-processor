...
AST.registerRuleProcessor('AST_ForIn', function processRule() {
	...
	var varRef,
		...
		getRef = function () {
			return varRef ? varRef : RuleProcessor.processRule(this.init);
		}.bind(this);
	...
	if (this.name) {
		RuleProcessor.processRule(this.init);
		varRef = Base.getIdentifierReference(context.lexicalEnvironment, this.name.name, context.strict);
	}

	experValue = Base.getValue(RuleProcessor.processRule(this.object));
	...
	if (experValueType === 'Unknown' || !Runtime.options.evaluateLoops) {
		...
		result = RuleProcessor.processRule(this.body);
		...
		while(obj) {
			...
			for(i = 0, len = propNames.length; i < len; i++) {
				...
				if (++loopIterations === Runtime.options.maxLoopIterations) {
					...
					stmt = RuleProcessor.processRule(this.body);
					if (stmt[1]) {
						v = stmt[1];
					}
					if (stmt[0] === 'continue') {

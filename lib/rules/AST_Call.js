...
/**
 * @event module:rules/AST_Call.rule
 ...
 * @property {module:base/types/function.FunctionType} func The function to be called. Only available post-evaluation.
 * @property {Array.<module:base.BaseType>} args The function arguments. Only available post-evaluation.
...
AST.registerRuleProcessor('AST_Call', function processRule() {
	...
	function runFunc(func, args, thisValue, runInSkippedMode) {
		...
		if (Base.type(func) === 'Unknown') {
			...
			if (runInSkippedMode) {
				if (func._ast && func._ast.start.file && !Runtime.isFileBlacklisted(func._ast.start.file)) {
					Base.processInSkippedMode(runFunc.bind(this, func, args, thisValue, runInSkippedMode));
				}
			} else {
				result = runFunc(func, args, thisValue, runInSkippedMode);
			}
		}

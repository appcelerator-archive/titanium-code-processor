	...
	for (i = 0, len = this.properties.length; i < len; i++) {
		...
		switch(prop.TYPE) {
			...
			case 'ObjectSetter':
				...
				if (Base.isSkippedMode()) {
					prop.value._skipped = !prop.value._visited;
					prop.value.name._skipped = !prop.value.name._visited;
				} else {
					prop.value._visited = true;
					prop.value._skipped = false;
					prop.value.name._visited = true;
					prop.value.name._skipped = false;
				}
				break;
			case 'ObjectGetter':
				propId = {

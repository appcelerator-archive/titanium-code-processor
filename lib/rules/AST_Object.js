...
AST.registerRuleProcessor('AST_Object', function processRule() {
	...
	for (; i < len; i++) {
		...
		switch(prop.TYPE) {
			case 'ObjectKeyVal':
				propId = {
					value: Base.getValue(RuleProcessor.processRule(prop.value)),
					writable: true,
					enumerable: true,
					configurable: true
				};
			...
			case 'ObjectSetter':
				propId = {
					enumerable: true,
					configurable: true,
					set: Base.getValue(RuleProcessor.processRule(prop.value))
				};
			...
			case 'ObjectGetter':
				propId = {
					enumerable: true,
					configurable: true,
					get: Base.getValue(RuleProcessor.processRule(prop.value))
				};
		...
		previous = obj.getOwnProperty(prop.key);
		if (previous &&
		...
		obj.defineOwnProperty(prop.key, propId, false);
	}

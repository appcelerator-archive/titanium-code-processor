...
AST.registerRuleProcessor('AST_Try', function processRule() {
	...
	function evalCatch(param) {
		...
		if (Base.isSkippedMode()) {
			catchBlock.argname._skipped = !catchBlock.argname._visited;
		} else {
			catchBlock.argname._visited = true;
			catchBlock.argname._skipped = false;
		}
	...
	if (catchBlock && finallyBlock) {
		if (Base.isSkippedMode()) {
			catchBlock._skipped = !catchBlock._visited;
			finallyBlock._skipped = !finallyBlock._visited;
		} else {
			catchBlock._visited = true;
			catchBlock._skipped = false;
			finallyBlock._visited = true;
			finallyBlock._skipped = false;
		}
		if (b[0] === 'throw') {

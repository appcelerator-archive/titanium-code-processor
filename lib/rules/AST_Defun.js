...
AST.registerRuleProcessor('AST_Defun', function processRule() {
	...
	this._lastKnownContext = context;

	// Check if this rule has already been processed (i.e. this is the evaluation pass, not the environment creation pass)
	if (context.lexicalEnvironment.envRec.hasBinding(identifier)) {
		functionObject = context.lexicalEnvironment.envRec.getBindingValue(identifier);
	} else {

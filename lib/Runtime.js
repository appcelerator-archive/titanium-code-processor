...
function getUnprocessedFilesList() {
	...
	for (i = 0, len = filesList.length; i < len; i++) {
		...
		if (jsRegex.test(filename) && processedFilesList.indexOf(path.resolve(path.join(parentDirectory, filename))) === -1) {
...
// ******** Methods requiring eventing ********

/**
 * @private
 */
on('fileProcessingBegin', function(e) {

...
function processRule(ast) {
	...
	var ruleName = getRuleName(ast),
		...
		result,
		currentLocation = Runtime.getCurrentLocation(),
		file = currentLocation ? currentLocation.file : '',
		line = currentLocation ? currentLocation.line : 1,
		column = currentLocation ? currentLocation.column : 1,
		i, len;
	...
	file = ast._filename || file;
	if (typeof ast[0] !== 'string') {
		Runtime.setCurrentLocation(file, line = ast[0].start.line + 1, ast[0].start.col + 1);
		locationStored = true;
	} else if (ast[0] === 'toplevel') {
		Runtime.setCurrentLocation(file, line = 1, 1);
		locationStored = true;
	}

	// Check if we are at a breakpoint
	for(i = 0, len = breakpoints.length; i < len; i++) {
		if (file.indexOf(breakpoints[i].file) !== -1 && line === breakpoints[i].line && column === breakpoints[i].column) {
			/*jshint debug: true*/
			debugger;
		}
	}
...
function isBlockStrict(astSet) {
	return !!astSet[0] && ((astSet[0][0].name === 'directive' && astSet[0][1] === 'use strict') ||
		(astSet[0][0].name === 'stat' && astSet[0][1][0].name === 'string' && astSet[0][1][1] === 'use strict'));
}

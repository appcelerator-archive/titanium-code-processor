	...
	AST.walk(ast, [
		{
			...
			callback: function(node) {
				...
				functions.push({
					...
					strict: RuleProcessor.isBlockStrict(node)
				});
				return true;
			}
		},
...
function createGlobalContext(ast, strict) {
	...
	for (i = 0, len = functions.length; i < len; i++) {
		...
		fo = new FunctionType(functions[i].formalParameterList, functions[i].functionBody,
			executionContext.lexicalEnvironment, strict || functions[i].strict);
...
function createEvalContext(callingContext, code, strict, isDirectEval) {
	...
	for (i = 0, len = functions.length; i < len; i++) {
		...
		fo = new FunctionType(functions[i].formalParameterList, functions[i].functionBody, executionContext.lexicalEnvironment,
			executionContext.strict || functions[i].strict);
		funcAlreadyDeclared = env.hasBinding(fn);
...
function createFunctionContext(functionObject, thisArg, argumentsList) {
	...
	Runtime.enterContext(executionContext);
	env = executionContext.variableEnvironment.envRec;
	result = findDeclarations(AST.createBodyContainer(functionObject.code));
	functions = result.functions;
	variables = result.variables;
...
EvalFunction.prototype.call = function call(thisVal, args, isDirectEval, filename) {
	...
	// Step 2
	ast = AST.parseString(x.value);
	if (ast.syntaxError) {
		handleRecoverableNativeException('SyntaxError', ast.message, {
				filename: Runtime.getCurrentLocation().filename,
				line: ast.line,
				column: ast.col
			});
		return new UnknownType();
	...
	// Step 4
	result = RuleProcessor.processRule(ast);

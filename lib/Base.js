...
function processFunctionDefinition(node) {
	...
	if (isSkippedMode()) {
		node.name._skipped = !node.name._visited;
	} else {
	...
	if (isSkippedMode()) {
		for (i = 0, len = node.argnames.length; i < len; i++) {
			node.argnames[i]._skipped = !node.argnames[i]._visited;
			formalParameterList.push(node.argnames[i].name);
		}
	} else {
		for (i = 0, len = node.argnames.length; i < len; i++) {
...
function getGlobalObject() {
	return isSkippedMode() ? clonedGlobalObject : globalObject;
}
...
function enterSkippedMode() {
	...
	if (!skippedModeCounter) {
		...
		for (i = 0, len = contextStack.length; i < len; i++) {
			clonedContextStack[i] = cloner.cloneContext(contextStack[i]);
		}
		clonedGlobalObject = clonedContextStack[0].thisBinding;
		prototypes.Object = clonedGlobalObject.get('Object').get('prototype');
		prototypes.Function = clonedGlobalObject.get('Function').get('prototype');
		prototypes.Number = clonedGlobalObject.get('Number').get('prototype');
		prototypes.Boolean = clonedGlobalObject.get('Boolean').get('prototype');
		prototypes.String = clonedGlobalObject.get('String').get('prototype');
		prototypes.Array = clonedGlobalObject.get('Array').get('prototype');
		prototypes.RegExp = clonedGlobalObject.get('RegExp').get('prototype');
		prototypes.Date = clonedGlobalObject.get('Date').get('prototype');
		prototypes.Error = clonedGlobalObject.get('Error').get('prototype');
		prototypes.EvalError = clonedGlobalObject.get('EvalError').get('prototype');
		prototypes.RangeError = clonedGlobalObject.get('RangeError').get('prototype');
		prototypes.ReferenceError = clonedGlobalObject.get('ReferenceError').get('prototype');
		prototypes.SyntaxError = clonedGlobalObject.get('SyntaxError').get('prototype');
		prototypes.TypeError = clonedGlobalObject.get('TypeError').get('prototype');
		prototypes.URIError = clonedGlobalObject.get('URIError').get('prototype');
	}
	skippedModeCounter++;
}
...
function exitSkippedMode() {
	skippedModeCounter--;
	if (!skippedModeCounter) {
		clonedContextStack = null;
		clonedGlobalObject = null;
		prototypes.Object = globalObject.get('Object').get('prototype');
		prototypes.Function = globalObject.get('Function').get('prototype');
		prototypes.Number = globalObject.get('Number').get('prototype');
		prototypes.Boolean = globalObject.get('Boolean').get('prototype');
		prototypes.String = globalObject.get('String').get('prototype');
		prototypes.Array = globalObject.get('Array').get('prototype');
		prototypes.RegExp = globalObject.get('RegExp').get('prototype');
		prototypes.Date = globalObject.get('Date').get('prototype');
		prototypes.Error = globalObject.get('Error').get('prototype');
		prototypes.EvalError = globalObject.get('EvalError').get('prototype');
		prototypes.RangeError = globalObject.get('RangeError').get('prototype');
		prototypes.ReferenceError = globalObject.get('ReferenceError').get('prototype');
		prototypes.SyntaxError = globalObject.get('SyntaxError').get('prototype');
		prototypes.TypeError = globalObject.get('TypeError').get('prototype');
		prototypes.URIError = globalObject.get('URIError').get('prototype');
	}
}
...
function getCurrentContext() {
	return isSkippedMode() ? clonedContextStack[clonedContextStack.length - 1] : contextStack[contextStack.length - 1];
}
...
function enterContext(context) {
	...
	(isSkippedMode() ? clonedContextStack : contextStack).push(context);
...
function exitContext() {
	...
	var context = (isSkippedMode() ? clonedContextStack : contextStack).pop();
	if (context.isFunctionContext) {
		functionContextCount--;
	}
	return context;
...
function getGlobalContext() {
	return isSkippedMode() ? clonedContextStack[0] : contextStack[0];
}
...
function getModuleContext() {
	return isSkippedMode() ? clonedContextStack[1] || clonedContextStack[0] : contextStack[1] || contextStack[0];
}
...
function handleRecoverableNativeException(exceptionType, message) {
	if ((isSkippedMode() || !inTryCatch()) && Runtime.options.nativeExceptionRecovery && !Runtime.options.exactMode) {

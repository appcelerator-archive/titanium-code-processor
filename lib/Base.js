...
function getGlobalObject() {
	return skippedModeCounter ? clonedGlobalObject : globalObject;
}
...
function getCurrentContext() {
	return skippedModeCounter ? clonedContextStack[clonedContextStack.length - 1] : contextStack[contextStack.length - 1];
}
...
function enterContext(context) {
	...
	(skippedModeCounter ? clonedContextStack : contextStack).push(context);
...
function exitContext() {
	...
	var context = (skippedModeCounter ? clonedContextStack : contextStack).pop();
	if (context.isFunctionContext) {
		functionContextCount--;
	}
	return context;
...
function getGlobalContext() {
	return skippedModeCounter ? clonedContextStack[0] : contextStack[0];
}
...
function getModuleContext() {
	return skippedModeCounter ? clonedContextStack[1] || clonedContextStack[0] : contextStack[1] || contextStack[0];
}

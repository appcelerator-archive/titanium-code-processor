...
BaseType.prototype._addProperty = function (p, desc) {
	...
	if (isSkippedMode()) {
		entry.alternateValues.push(desc);
	} else {
		entry.value = desc;
	}
};
	...
	function lookup(desc) {
		...
		if (desc) {
			if (isDataDescriptor(desc)) {
				...
				value = desc.get && desc.get.className !== 'Undefined' ? desc.get.callFunction(this) : new UndefinedType();
			}
		}
		return value;
	}
...
ObjectType.prototype.getOwnProperty = function getOwnProperty(p, alternate) {
	...
	if (x) {
		if (alternate) {
			return x.map(function (desc) {
				return copyDescriptor(desc);
			});
		} else {
...
function enterSkippedMode() {
	skippedModeStack.push(++skippedModeCounter);
}
 ...
 *
 * @method
 * @returns {Boolean} Whether or not we are in skipped mode
 */
exports.isSkippedMode = isSkippedMode;
function isSkippedMode() {
	return !!skippedModeStack.length;
}

/**
 * Gets the current skipped mode section identifier
 *
 * @method
 * @returns {Number} The section identifier
 */
exports.getSkippedSection = getSkippedSection;
function getSkippedSection() {
	return skippedModeStack[skippedModeStack.length - 1];
}

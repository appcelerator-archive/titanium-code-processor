...
BaseType.prototype._isLocal = function () {
	...
	while (lexicalEnvironment) {
		if (targetLexicalEnvironment === lexicalEnvironment) {
			return true;
		} else if (lexicalEnvironment.ambiguous) {
			return false;
		}
		lexicalEnvironment = lexicalEnvironment.outer;
	}
	throw new Error('Invalid closure on value, this is a bug in the code processor');
};
...
ObjectType.prototype.defineOwnProperty = function defineOwnProperty(p, desc, throwFlag, suppressEvent) {
	...
	if (isDataDescriptor(desc)) {
		...
		if (type(desc.value) === 'Unknown' || !desc.value._isLocal() || Runtime.ambiguousBlock) {
 ...
 */
FunctionType.prototype.call = function call(thisVal, args, ambiguous) {
	...
	if (type(thisVal) === 'Unknown') {
		...
		funcCtx.lexicalEnvironment.ambiguous = !!ambiguous || !!Runtime.ambiguousBlock;
...
DeclarativeEnvironmentRecord.prototype.setMutableBinding = function setMutableBinding(n, v, s) {
	...
	if (type(v) === 'Unknown' || !v._isLocal() || Runtime.ambiguousBlock) {
...
EvalFunction.prototype.call = function call(thisVal, args, isDirectEval) {
	...
	// Do the analysis coverage computation
	AST.walk(prog, { 
	'*': function(node, next) {
		if (node._visited) {
			numNodesVisited++;
	...
	this._numNodesVisited = numNodesVisited;
	this._numTotalNodes = numTotalNodes;

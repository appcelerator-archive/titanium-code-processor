...
function throwNativeException (exceptionType, message) {
	throwException(globalObjects[exceptionType].construct([new StringType(message)]));
}
...
function throwException (exception) {
	...
	// Set the exception
	exception.file = Runtime.getCurrentFile();
...
function BaseType(className) {
	...
	this._creationContext = Runtime.getCurrentContext();
	this._leaked = false;
}
util.inherits(BaseType, Runtime.Evented);

/**
 * Checks if this value is completely local to the current context/child contexts
 * 
 * @private
 */
BaseType.prototype._isLocal = function () {
	var currentContext = Runtime.getCurrentContext();
	return !this._leaked && (!this._creationContext || this._creationContext === currentContext ||
		currentContext === Runtime.getGlobalContext() || currentContext === Runtime.getModuleContext());
};
...
ObjectType.prototype.getOwnProperty = function getOwnProperty(p) {
	...
	if (x) {
		d = {};
		if (Runtime.ambiguousCode && ((isDataDescriptor(x) && x.writable) || (isAccessorDescriptor(x) && x.set && isCallable(x.set)))) {
			d = {
				value: new UnknownType(),
				writable: !!x.writable
			}
...
ObjectType.prototype.defineOwnProperty = function defineOwnProperty(p, desc, throwFlag, suppressEvent) {
	...
	if (!this._isLocal() && isDataDescriptor(desc) && type(desc.value) === 'Object') {
		desc.value._leaked = true;
		console.log('Value being set to ' + p + ' has been leaked');
	}
...
NumberProtoToStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (type(thisVal) !== 'Number') {
		if (type(thisVal) === 'Object' && thisVal.className === 'Number') {
			...
			throwNativeException('TypeError', 'Value is not a number or number object');
		}
	}
...
BooleanProtoToStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (type(thisVal) !== 'Boolean') {
		if (type(thisVal) === 'Object' && thisVal.className === 'Boolean') {
			...
			throwNativeException('TypeError', 'Value is not a boolean or boolean object');
		}
	} else {
		...
		if (indx < names.length) {
			...
			if (!strict && !(name in mappedNames)) {
				...
				map.defineOwnProperty(indx, {
					...
					get: new FunctionType([], AST.parseString('function temp () { return ' + name + '; }')[1][0][3], env, true),
					set: new FunctionType([name + '_arg'], AST.parseString(name + ' = ' + name + '_arg;')[1], env, true),
					configurable: true
				}, false, true);
			}
		}
...
EvalFunction.prototype.call = function call(thisVal, args, isDirectEval) {
	...
	// Step 3
	evalCtx = createEvalContext(Runtime.getCurrentContext(), prog, RuleProcessor.isBlockStrict(prog[1]), isDirectEval);

...
function UnknownType(typeHint, className) {
...
/**
 * defineProperty() prototype method
 *
 * @private
 * @see ECMA-262 Spec Chapter 15.2.3.2
 */
...
ObjectDefinePropertyFunc.prototype.call = function call(thisVal, args) {
	...
	if (type(p) === 'Unknown' || type(attributes) === 'Unknown') {
		toUnknown(o);
		return new UnknownType();
	}
...
ObjectIsSealedFunc.prototype.call = function call(thisVal, args) {
	...
	if (type(o) !== 'Object') {
		...
		return new UnknownType('Boolean');
	}
...
RegExpConstructor.prototype.call = function call(thisVal, args) {
	...
	if (type(pattern) === 'Unknown') {
		return new UnknownType('RegExp');
	}
...
RegExpConstructor.prototype.construct = function call(args) {
	...
	if (type(pattern) === 'Unknown' || type(flags) === 'Unknown') {
		return new UnknownType('RegExp');
	}
...
FunctionConstructor.prototype.construct = function call(args) {
	...
	// Validate the parameters
	if (areAnyUnknown(args)) {
		return new UnknownType('Function');
	}
	...
	if (body.syntaxError) {
		handleRecoverableNativeException('SyntaxError');
		return new UnknownType('Functon');
	}
	body = body.body[0];
...
DateUTCFunc.prototype.call = function call(thisVal, args) {
	return new UnknownType(args.length > 1 ? 'Number' : undefined);
};
...
/**
 * Converts the given object to an unknown value
 *
 * @method
 * @name module:Base.toUnknown
 * @param {module:Base.BaseType} input The value to convert
 */
exports.toUnknown = toUnknown;
function toUnknown(input) {
	input.className = input.type = 'Unknown';
	input.typeHint = undefined;
	Runtime._unknown = true;
}
...
function toString(input) {
	...
	if (type(input) === 'Unknown') {
		newString = new UnknownType('String');

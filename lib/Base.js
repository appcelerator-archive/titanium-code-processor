...
function injectGlobals() {
	...
	// Properties
	addProperty('NaN', new NumberType(NaN));
	addProperty('Infinity', new NumberType(Infinity));
...
function toPropertyDescriptor(obj) {
	...
	if (type(obj) === 'Unknown') {
		...
		if ((desc.get || desc.set) && (isDefined(desc.value) || isDefined(desc.writable))) {
			throwNativeException('TypeError', 'Property descriptors cannot contain both get/set and value/writeable properties');
		}
	} else {
...
ObjectType.prototype.put = function put(p, v, throwFlag, suppressEvent) {
	...
	if (!canPutP) {
		if (throwFlag) {
			throwNativeException('TypeError', 'Cannot put argument');
		} else {
			return;
		}
	}
	...
	if (desc) {
		if (isAccessorDescriptor(desc)) {
			if (Runtime.ambiguousCode) {
				return desc.set.className !== 'Undefined' ? 'Unknown' : false;
			} else {
...
ObjectType.prototype.defaultValue = function defaultValue(hint) {
	...
	if (hint === 'String') {
		...
		throwNativeException('TypeError', 'Could not get the default string value');
	} else {
		result = defaultValueOf.call(this);
		if (result) {
			return result;
		...
		throwNativeException('TypeError', 'Could not get the default number value');
	}
};
...
ObjectType.prototype.defineOwnProperty = function defineOwnProperty(p, desc, throwFlag, suppressEvent) {
	...
	if (isUndefined(current) && !this.extensible) {
		if (throwFlag) {
			throwNativeException('TypeError', 'Could not define property ' + p + ': object is not extensible');
		}
		return false;
	}
	...
	if (!current.configurable) {
		if (desc.configurable || (isDefined(desc.enumerable) && desc.enumerable !== current.enumerable)) {
			if (throwFlag) {
				throwNativeException('TypeError', 'Could not define property ' + p + 
		...
		if (hasPrimitiveBase(v)) {
			put = function put(p, w, throwFlag) {
				...
				if (!canPutP || isDataDescriptor(o.getOwnProperty(p))) {
					if (throwFlag) {
						throwNativeException('TypeError', 'Could not put ' + v.referencedName);
					}
					return;
				}
				desc = o.getProperty(p);
...
ObjectProtoToLocaleStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (type(toString) === "Unknown") {
		...
		throwNativeException('TypeError', 'toString is not callable');
	}
...
function ObjectPrototypeType(className) {
	...
	this.put("constructor", GlobalObjects['Object'], false, true);
...
util.inherits(ObjectPrototypeType, ObjectType);// ******** Object Constructor ********
...
GlobalObjects['Object'] = new ObjectConstructor();
...
function FunctionPrototypeType(className) {
	...
	this.put("constructor", GlobalObjects['Function'], false, true);
...
ArrayProtoToLocaleStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (isType(firstElement, ["Undefined", "Null"])) {
		...
		if (!isCallable(func)) {
			throwNativeException('TypeError', 'toLocaleString is not callable');
		}
	...
	function sortCompare(j, k) {
		...
		if (compareFn && type(compareFn) !== "Undefined") {
			...
			if (!isCallable(compareFn)) {
				throwNativeException('TypeError', 'Compare funciton is not callable');
			}
...
ArrayProtoEveryFunc.prototype.call = function call(thisVal, args) {
	...
	if (!isCallable(callbackFn)) {
		throwNativeException('TypeError', 'Callback function is not callable');
	}
...
ArrayProtoReduceFunc.prototype.call = function call(thisVal, args) {
	...
	if (len === 0 && !initialValue) {
		throwNativeException('TypeError', 'Missing initial value');
	}
	...
	if (initialValue) {
		...
		if (!kPresent) {
			throwNativeException('TypeError', 'Missing property ' + k);
		}
	}
...
function ArrayPrototypeType(className) {
	...
	this.put("constructor", GlobalObjects['Array'], false, true);
...
GlobalObjects['Array'] = new ArrayConstructor();
...
StringConstructor.prototype.construct = function call(args) {
	...
	obj.objectPrototype = new StringPrototypeType();
	
	return obj;
};
GlobalObjects['String'] = new StringConstructor();
...
function BooleanPrototypeType(className) {
	...
	this.put("constructor", GlobalObjects['Boolean'], false, true);
...
NumberProtoToStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (type(thisVal) !== "Number") {
		throwNativeException('TypeError', 'Value is not a number object');
	}
	...
	if (radix && type(radix) !== "Undefined") {
		...
		if (radix < 2 || radix > 36) {
			throwNativeException('RangeError', 'Invalid radix value ' + radix);
		}
	} else {
		radix = undefined;
	}
...
NumberProtoToFixedFunc.prototype.call = function call(thisVal, args) {
	...
	// Step 2
	if (f < 0 || f > 20) {
		throwNativeException('RangeError', 'Invalid fraction digits value ' + f);
	}
...
NumberProtoToPrecisionFunc.prototype.call = function call(thisVal, args) {
	...
	// Step 2
	if (p < 1 || p > 21) {
		throwNativeException('RangeError', 'Invalid precision value ' + p);
	}
...
function NumberPrototypeType(className) {
	...
	this.put("constructor", GlobalObjects['Number'], false, true);
...
function NumberConstructor(className) {
	...
	this.put('MAX_VALUE', new NumberType(Number.MAX_VALUE), false, true);
	this.put('MIN_VALUE', new NumberType(Number.MIN_VALUE), false, true);
	this.put('NaN', new NumberType(NaN), false, true);
	this.put('NEGATIVE_INFINITY', new NumberType(Number.NEGATIVE_INFINITY), false, true);
	this.put('POSITIVE_INFINITY', new NumberType(Number.POSITIVE_INFINITY), false, true);
}
...
NumberConstructor.prototype.construct = function call(args) {
	...
	obj.objectPrototype = new NumberPrototypeType();
		
	return obj;
};
GlobalObjects['Number'] = new NumberConstructor();
...
function RegExpPrototypeType(className) {
	...
	this.put("constructor", GlobalObjects['RegExp'], false, true);
...
GlobalObjects['RegExp'] = new RegExpConstructor();
...
function ErrorPrototypeType(className) {
	...
	this.put("constructor", GlobalObjects['Error'], false, true);
...
MathPowFunc.prototype.call = function call(thisVal, args) {
	...
	if (!x || !y) {
		...
		return new NumberType(Math.log(toNumber(x).value), Math.log(toNumber(y).value));
	}
};
...
function MathObject(className) {
	...
	// Properties
	put('E', new NumberType(Math.E));
	put('LN10', new NumberType(Math.LN10));
	put('LN2', new NumberType(Math.LN2));
	put('LOG2E', new NumberType(Math.LOG2E));
	put('LOG10E', new NumberType(Math.LOG10E));
	put('PI', new NumberType(Math.PI));
	put('SQRT1_2', new NumberType(Math.SQRT1_2));
	put('SQRT2', new NumberType(Math.SQRT2));
...
JSONParseFunc.prototype.call = function call(thisVal, args) {
	...
	try {
		...
		throwNativeException('SyntaxError', e.message);
	}

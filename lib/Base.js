...
exports.toPropertyDescriptor = function toPropertyDescriptor(obj) {
	...
	if ((desc.get || desc.set) && (desc.value !== undefined || desc.writable !== undefined)) {
		throw new Exceptions.TypeError();
	}
	return desc;
};
 ...
 */
exports.ObjectType = function ObjectType(className, value, dontCreatePrototype) {
	
	// Step 1
	if (value && isObject(value)) {
		return value;
	} else if(value && ~["String", "Number", "Boolean"].indexOf(value)) {
...
exports.ObjectType.prototype.defineOwnProperty = function defineOwnProperty(p, desc, throwFlag) {
	...
	if (!current.configurable) {
		if (desc.configurable || (desc.enumerable !== undefined && desc.enumerable !== current.enumerable)) {
			if (throwFlag) {
				throw new Exceptions.TypeError();
			}
			return false;
	...
	if (isGenericDescriptor(desc)) {
		...
		if (!current.configurable) {
			...
			if (!current.writable && desc.value !== undefined && !sameDesc(desc, current)) {
				if (throwFlag) {
					throw new Exceptions.TypeError();
				}
				return false;
		...
		if (!current.configurable) {
			if (desc.set !== undefined && !sameDesc(desc.set, current.set)) {
				if (throwFlag) {
					throw new Exceptions.TypeError();
				}
				return false;
			}
			if (desc.get !== undefined && !sameDesc(desc.get, current.get)) {
				if (throwFlag) {
					throw new Exceptions.TypeError();
				}
				return false;
 ...
 */
exports.ArrayType = function ArrayType(className) {
...
exports.isPropertyReference = function isPropertyReference(v) {
	return hasPrimitiveBase(v) || !!~["Object", "Function", "Arguments", "Array"].indexOf(type(getBase(v)));
};
 ...
 */
exports.ObjectEnvironmentRecord.prototype.createMutableBinding = function createMutableBinding(n, d) {
 ...
 */
exports.ObjectEnvironmentRecord.prototype.setMutableBinding = function setMutableBinding(n, v, s) {
	...
	if (mappedNames.length) {
		...
		obj.get = function get(p) {
			...
			if (isMapped) {
				...
				if (p === "callee" && type(v) === "Function" && v.strict) {
					throw new Exceptions.TypeError();
				}
				return v;
			}
...
exports.createFunctionContext = function createFunctionContext(functionObject, thisArg, argumentsList) {
	...
	if (functionObject.strict) {
		...
		thisBinding = Runtime.getGlobalContex(); // Use the "global" object as created in "toplevel," equivalent to module scope in node.js
 ...
 */  
exports.FunctionTypeBase = function FunctionTypeBase(length, dontCreatePrototype, className) {
...
var FunctionTypeBase = exports.FunctionTypeBase;
 ...
 * @see ECMA-262 Spec Chapter 13.2
 */
exports.FunctionType = function FunctionType(formalParameterList, functionBody, lexicalEnvironemnt, strict, className) {
...
var throwTypeError;
exports.createThrowTypeErrorFunction = function createThrowTypeErrorFunction() {
...
util.inherits(ObjectProtoToStringFunc, FunctionType);
ObjectProtoToStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (type(thisVal) === "Undefined") {
		...
		result.value = "[object " + type(toObject(thisVal)) + "]";
	}
	
	return result;
};
ObjectProtoToStringFunc.prototype.construct = false;
...
util.inherits(ObjectProtoToLocaleStringFunc, FunctionType);
...
ObjectProtoToLocaleStringFunc.prototype.construct = false;
...
util.inherits(ObjectProtoValueOfFunc, FunctionType);
ObjectProtoValueOfFunc.prototype.call = function call(thisVal, args) {
	return toObject(thisVal);
};
ObjectProtoValueOfFunc.prototype.construct = false;
...
util.inherits(ObjectProtoHasOwnPropertyFunc, FunctionType);
...
ObjectProtoHasOwnPropertyFunc.prototype.construct = false;
...
util.inherits(ObjectProtoIsPrototypeOfFunc, FunctionType);
ObjectProtoIsPrototypeOfFunc.prototype.call = function call(thisVal, args) {
	...
	if (isObject(v)) {
		...
		while(true) {
			v = v.objectPrototype;
			if (!v || type(v) === "Undefined" || type(v) === "Null") {
				break;
			}
			if (o === v) {
				result.value = true;
...
ObjectProtoIsPrototypeOfFunc.prototype.construct = false;
...
util.inherits(ObjectProtoPropertyIsEnumerableFunc, FunctionType);
ObjectProtoPropertyIsEnumerableFunc.prototype.call = function call(thisVal, args) {
	var result = new BooleanType(),
		p = toString(args[0]),
		o = toObject(thisVal),
		desc = o.getOwnProperty(p.value);
		result.value = desc !== undefined && desc.enumerable;
};
ObjectProtoPropertyIsEnumerableFunc.prototype.construct = false;
 ...
 */
exports.ObjectPrototypeType = function ObjectPrototypeType(className) {
	...
	// toString method
	this.put("toString", new ObjectProtoToStringFunc(), false);
	this.put("toLocaleString", new ObjectProtoToLocaleStringFunc(), false);
	this.put("valueOf", new ObjectProtoValueOfFunc(), false);
	this.put("hasOwnProperty", new ObjectProtoHasOwnPropertyFunc(), false);
	this.put("isPrototypeOf", new ObjectProtoIsPrototypeOfFunc(), false);
	this.put("propertyIsEnumerable", new ObjectProtoPropertyIsEnumerableFunc(), false);
};
var ObjectPrototypeType = exports.ObjectPrototypeType;
...
util.inherits(FunctionProtoToStringFunc, FunctionType);
FunctionProtoToStringFunc.prototype.call = function call(thisVal, args) {
	if (type(thisVal) !== "Function") {
		throw new Exceptions.TypeError();
	}
	return ObjectProtoToStringFunc.apply(this, arguments);
};
FunctionProtoToStringFunc.prototype.construct = false;
...
util.inherits(FunctionProtoApplyFunc, FunctionType);
FunctionProtoApplyFunc.prototype.call = function call(thisVal, args) {
	...
	if (!argArray || type(argArray) === "Undefined" || type(argArray) === "Null") {
...
FunctionProtoApplyFunc.prototype.construct = false;
...
util.inherits(FunctionProtoCallFunc, FunctionType);
...
FunctionProtoCallFunc.prototype.construct = false;
...
util.inherits(FunctionProtoBindFunc, FunctionType);
FunctionProtoBindFunc.prototype.call = function call(thisVal, args) {
	...
	l = new NumberType();
	l.value = type(target) === "Function" ? Math.max(0, target.get("length").value - a.length) : 0;
	f.put("length", l);
...
FunctionProtoBindFunc.prototype.construct = false;
 ...
 */
exports.FunctionPrototypeType = function FunctionPrototypeType(className) {
	
	// Warning: setting the third argument to anything falsey, or leaving it off, results in infinite recursion
	FunctionTypeBase.call(this, className || "Function", true);
	
	this.put("toString", new FunctionProtoToStringFunc(), false);
	this.put("apply", new FunctionProtoApplyFunc(), false);
	this.put("call", new FunctionProtoCallFunc(), false);
	this.put("bind", new FunctionProtoBindFunc(), false);
};
var FunctionPrototypeType = exports.FunctionPrototypeType;
...
util.inherits(ArrayProtoToStringFunc, FunctionType);
...
ArrayProtoToStringFunc.prototype.construct = false;
...
util.inherits(ArrayProtoToLocaleStringFunc, FunctionType);
ArrayProtoToLocaleStringFunc.prototype.call = function call(thisVal, args) {
	...
	// Steps 7 and 8
	if (type(firstElement) === "Undefined" || type(firstElement) === "Null") {
		r = "";
	} else {
...
ArrayProtoToLocaleStringFunc.prototype.construct = false;
...
util.inherits(ArrayProtoConcatFunc, FunctionType);
ArrayProtoConcatFunc.prototype.call = function call(thisVal, args) {
	...
	a.put("length", lengthNumber, false);
	
	// Step 6
	return a;
};
ArrayProtoConcatFunc.prototype.construct = false;
...
util.inherits(ArrayProtoJoinFunc, FunctionType);
...
ArrayProtoJoinFunc.prototype.construct = false;
...
util.inherits(ArrayProtoPopFunc, FunctionType);
ArrayProtoPopFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoPopFunc.prototype.construct = false;
...
util.inherits(ArrayProtoPushFunc, FunctionType);
ArrayProtoPushFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoPushFunc.prototype.construct = false;
...
util.inherits(ArrayProtoReverseFunc, FunctionType);
ArrayProtoReverseFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoReverseFunc.prototype.construct = false;
...
util.inherits(ArrayProtoShiftFunc, FunctionType);
ArrayProtoShiftFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoShiftFunc.prototype.construct = false;
...
util.inherits(ArrayProtoSliceFunc, FunctionType);
ArrayProtoSliceFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoSliceFunc.prototype.construct = false;
...
util.inherits(ArrayProtoSortFunc, FunctionType);
ArrayProtoSortFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoSortFunc.prototype.construct = false;
...
util.inherits(ArrayProtoSpliceFunc, FunctionType);
ArrayProtoSpliceFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoSpliceFunc.prototype.construct = false;
...
util.inherits(ArrayProtoUnshiftFunc, FunctionType);
ArrayProtoUnshiftFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoUnshiftFunc.prototype.construct = false;
...
util.inherits(ArrayProtoIndexOfFunc, FunctionType);
ArrayProtoIndexOfFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoIndexOfFunc.prototype.construct = false;
...
util.inherits(ArrayProtoLastIndexOfFunc, FunctionType);
ArrayProtoLastIndexOfFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoLastIndexOfFunc.prototype.construct = false;
...
util.inherits(ArrayProtoEveryFunc, FunctionType);
ArrayProtoEveryFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoEveryFunc.prototype.construct = false;
...
util.inherits(ArrayProtoSomeFunc, FunctionType);
ArrayProtoSomeFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoSomeFunc.prototype.construct = false;
...
util.inherits(ArrayProtoForEachFunc, FunctionType);
ArrayProtoForEachFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoForEachFunc.prototype.construct = false;
...
util.inherits(ArrayProtoMapFunc, FunctionType);
ArrayProtoMapFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoMapFunc.prototype.construct = false;
...
util.inherits(ArrayProtoFilterFunc, FunctionType);
ArrayProtoFilterFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoFilterFunc.prototype.construct = false;
...
util.inherits(ArrayProtoReduceFunc, FunctionType);
ArrayProtoReduceFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayProtoReduceFunc.prototype.construct = false;
...
util.inherits(ArrayReduceRightFunc, FunctionType);
ArrayReduceRightFunc.prototype.call = function call(thisVal, args) {
	// TODO:
	throw new Error("IMPLEMENT ME");
};
ArrayReduceRightFunc.prototype.construct = false;
 ...
 */
exports.ArrayPrototypeType = function ArrayPrototypeType(className) {
	ObjectType.call(this, className);
	
	this.put("toString", new ArrayProtoToStringFunc(), false);
	this.put("toLocaleString", new ArrayProtoToLocaleStringFunc(), false);
	this.put("concat", new ArrayProtoConcatFunc(), false);
	this.put("join", new ArrayProtoJoinFunc(), false);
	this.put("pop", new ArrayProtoPopFunc(), false);
	this.put("push", new ArrayProtoPushFunc(), false);
	this.put("reverse", new ArrayProtoReverseFunc(), false);
	this.put("shift", new ArrayProtoShiftFunc(), false);
	this.put("slice", new ArrayProtoSliceFunc(), false);
	this.put("sort", new ArrayProtoSortFunc(), false);
	this.put("splice", new ArrayProtoSpliceFunc(), false);
	this.put("unshift", new ArrayProtoUnshiftFunc(), false);
	this.put("indexOf", new ArrayProtoIndexOfFunc(), false);
	this.put("lastIndexOf", new ArrayProtoLastIndexOfFunc(), false);
	this.put("every", new ArrayProtoEveryFunc(), false);
	this.put("some", new ArrayProtoSomeFunc(), false);
	this.put("forEach", new ArrayProtoForEachFunc(), false);
	this.put("map", new ArrayProtoMapFunc(), false);
	this.put("filter", new ArrayProtoFilterFunc(), false);
	this.put("reduce", new ArrayProtoReduceFunc(), false);
	this.put("reduceRight", new ArrayReduceRightFunc(), false);
	
};
var ArrayPrototypeType = exports.ArrayPrototypeType;

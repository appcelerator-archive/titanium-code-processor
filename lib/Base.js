...
function enterAmbiguousBlock() {
	Runtime.getCurrentContext()._ambiguousBlock++;
}
...
function isAmbiguousBlock() {
	return !!Runtime.getCurrentContext()._ambiguousBlock;
}
...
EvalFunction.prototype.callFunction = function callFunction(thisVal, args, options) {
	...
	// Step 3
	createEvalContext(Runtime.getCurrentContext(), ast, RuleProcessor.isBlockStrict(ast), options && options.isDirectEval);
...
function throwNativeException (exceptionType, message) {
	var exc = getValue(getIdentifierReference(Runtime.getGlobalContext().variableEnvironment, exceptionType, false));

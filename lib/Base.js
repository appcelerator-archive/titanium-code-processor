...
function throwException (exceptionType, message) {
	...
	// Runtime.exception is pulled out.
...
function UnknownType(className) {
	if (Runtime.options.exactMode) {
		throw new Error('Attempted to instantiate an unknown type in exact mode');
	}
	...
	if (type(obj) === 'Unknown') {
		...
		if (obj.hasProperty('get')) {
			...
			if (type(getter) !== 'Undefined' && type(getter) !== 'Unknown' && !isCallable(getter)) {
				throwException('TypeError', 'get is not callable');
			}
			desc.get = getter;
		}
		if (obj.hasProperty('set')) {
			...
			if (type(setter) !== 'Undefined' && type(setter) !== 'Unknown' && !isCallable(setter)) {
				throwException('TypeError', 'set is not callable');
			}
			desc.set = setter;
		}
		if ((desc.get || desc.set) && (isDefined(desc.value) || isDefined(desc.writable))) {
			throwException('TypeError', 'Property descriptors cannot contain both get/set and value/writeable properties');
		}
	} else {
		throwException('TypeError', 'Property descriptors must be objects');
	}
...
ObjectType.prototype.get = function get(p) {
	...
	if (desc) {
		if (isDataDescriptor(desc)) {
			if (Runtime.ambiguousCode && desc.writable) {
				result = new UnknownType();
			} else {
				result = desc.value;
			}
		} else {
			if (Runtime.ambiguousCode && desc.set && isCallable(desc.set)) {
				result = new UnknownType();
			} else {
				result = desc.get.className !== 'Undefined' ? desc.get.call(this) : new UndefinedType();
			}
		}
	} else if (Runtime.ambiguousCode) {
		result = new UnknownType();
	}
...
ObjectType.prototype.getOwnProperty = function getOwnProperty(p) {
	...
	if (x) {
		d = {};
		if (Runtime.ambiguousCode && ((isDataDescriptor(x) && x.writeable) || (isAccessorDescriptor(x) && x.set && isCallable(x.set)))) {
			d = {
				value: new UnknownType(),
				writeable: !!x.writable
			}
		} else if (isDataDescriptor(x)) {
			d.value = x.value;
...
ObjectType.prototype.put = function put(p, v, throwFlag, suppressEvent) {
	...
	if (!canPutP) {
		if (throwFlag) {
			throwException('TypeError', 'Cannot put argument');
		} else {
			return;
		}
	}
	...
	if (desc) {
		if (isAccessorDescriptor(desc)) {
			if (Runtime.ambiguousCode) {
				return desc.set && desc.set.className !== 'Undefined' ? 'Unknown' : false;
			} else {
				return desc.set && desc.set.className !== 'Undefined';
			}
		} else {
			return Runtime.ambiguousCode && desc.writable ? 'Unknown' : desc.writable;
		}
	}
...
ObjectType.prototype['delete'] = function objDelete(p, throwFlag) {
	...
	if (throwFlag) {
		throwException('TypeError', 'Unable to delete "' + p + '"');
	}
	return false;
};
...
ObjectType.prototype.defaultValue = function defaultValue(hint) {
	...
	if (hint === 'String') {
		...
		throwException('TypeError', 'Could not get the default string value');
	} else {
		result = defaultValueOf.call(this);
		if (result) {
			return result;
		...
		throwException('TypeError', 'Could not get the default number value');
	}
};
...
ObjectType.prototype.defineOwnProperty = function defineOwnProperty(p, desc, throwFlag, suppressEvent) {
	...
	if (isUndefined(current) && !this.extensible) {
		if (throwFlag) {
			throwException('TypeError', 'Could not define property ' + p + ': object is not extensible');
		}
		return false;
	}
	...
	if (!current.configurable) {
		if (desc.configurable || (isDefined(desc.enumerable) && desc.enumerable !== current.enumerable)) {
			if (throwFlag) {
				throwException('TypeError', 'Could not define property ' + p + 
...
function RegExpType(pattern, flags, className) {
	...
	if (isDefined(pattern)) {
		try {
			...
			throwException('SyntaxError', 'Regular expression pattern is undefined');
		}
...
function getValue(v) {
	...
	if (isUnresolvableReference(v)) {
		throwException('ReferenceError', v.referencedName + ' is not resolvable');
	}
...
function putValue(v, w) {
	...
	if (type(v) !== 'Reference') {
		throwException('ReferenceError', 'Attempted to put a value to a non-reference');
	}
	...
	if (isUnresolvableReference(v)) {
		...
		Runtime.getModuleObject().put(getReferencedName(v), w, false);
		...
		if (hasPrimitiveBase(v)) {
			put = function put(p, w, throwFlag) {
				...
				if (!canPutP || isDataDescriptor(o.getOwnProperty(p))) {
					if (throwFlag) {
						throwException('TypeError', 'Could not put ' + v.referencedName);
					}
					return;
				}
				desc = o.getProperty(p);
...
NumberProtoToStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (type(thisVal) !== 'Number') {
		throwException('TypeError', 'Value is not a number object');
	}
	...
	if (radix && type(radix) !== 'Undefined') {
		...
		if (radix < 2 || radix > 36) {
			throwException('RangeError', 'Invalid radix value ' + radix);
		}
	} else {
		radix = undefined;
	}
...
NumberProtoToFixedFunc.prototype.call = function call(thisVal, args) {
	...
	// Step 2
	if (f < 0 || f > 20) {
		throwException('RangeError', 'Invalid fraction digits value ' + f);
	}
...
NumberProtoToPrecisionFunc.prototype.call = function call(thisVal, args) {
	...
	// Step 2
	if (p < 1 || p > 21) {
		throwException('RangeError', 'Invalid precision value ' + p);
	}
...
BooleanProtoToStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (type(b) !== 'Boolean') {
		if (type(b) === 'Object' && b.className === 'Boolean') {
			...
			throwException('TypeError', 'Value is not a boolean object');
		}
	}
...
StringProtoToStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (thisVal.className !== 'String') {
		throwException('TypeError', 'Value is not a string');
	}
...
ObjectProtoToLocaleStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (type(toString) === 'Unknown') {
		...
		throwException('TypeError', 'toString is not callable');
	}
...
ArrayProtoToLocaleStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (isType(firstElement, ['Undefined', 'Null'])) {
		...
		if (!isCallable(func)) {
			throwException('TypeError', 'toLocaleString is not callable');
		}
	...
	function sortCompare(j, k) {
		...
		if (compareFn && type(compareFn) !== 'Undefined') {
			...
			if (!isCallable(compareFn)) {
				throwException('TypeError', 'Compare funciton is not callable');
			}
...
ArrayProtoEveryFunc.prototype.call = function call(thisVal, args) {
	...
	if (!isCallable(callbackFn)) {
		throwException('TypeError', 'Callback function is not callable');
	}
...
ArrayProtoReduceFunc.prototype.call = function call(thisVal, args) {
	...
	if (len === 0 && !initialValue) {
		throwException('TypeError', 'Missing initial value');
	}
	...
	if (initialValue) {
		...
		if (!kPresent) {
			throwException('TypeError', 'Missing property ' + k);
		}
	}
...
FunctionProtoToStringFunc.prototype.call = function call(thisVal, args) {
	...
	if (thisVal.className !== 'Function') {
		throwException('TypeError', 'Cannot invoke non-function type');
	}
...
FunctionProtoApplyFunc.prototype.call = function call(thisVal, args) {
	...
	if (!isCallable(thisVal)) {
		throwException('TypeError', 'Attempted to call non-callable value');
	}
	...
	if (!isObject(argArray)) {
		throwException('TypeError', 'Arguments value is not an object');
	}
...
FunctionProtoBindFunc.prototype.call = function call(thisVal, args) {
	...
	f.construct = function construct(extraArgs) {
		if (!target.construct) {
			throwException('TypeError', 'Bind target does not have a constructor');
		}
	...
	f.hasInstance = function hasInstance(v) {
		if (!target.hasInstance) {
			throwException('TypeError', 'Bind target does not have a hasInstance method');
		}
...
function ErrorPrototypeType(errorType, className) {
	FunctionTypeBase.call(this, 0, false, className);
	...
	if (args.length === 1) {
		...
		if (type(len) === 'Number') {
			if (len.value === toUint32(len).value) {
				...
				throwException('RangeError', 'Invalid length ' + len.value);
			}
		} else {
...
prototypes['Array'].put('constructor', globalObjects['RegExp'] = new RegExpConstructor(), false, true);
...
function DateConstructor(className) {
	FunctionTypeBase.call(this, 7, false, className || 'Function');
...
function toObject(input) {
	...
	switch (type(input)) {
		...
		case 'Unknown':
			return new UnknownType();
		default:
			throwException('TypeError', 'Values of type ' + type(input) + ' cannot be converted to objects');
	}
	return newObject;
}
...
function checkObjectCoercible(input) {
	if (isType(input, ['Undefined', 'Null'])) {
		throwException('TypeError', 'Invalid type: ' + type(input).toLowerCase());
	}
	return;
}
...
DeclarativeEnvironmentRecord.prototype.setMutableBinding = function setMutableBinding(n, v, s) {
	...
	if (!bindings[n].isMutable) {
		if (s) {
			throwException('TypeError', 'Could not set binding: binding "' + n + '" is not mutable');
		} else {
			return;
		}
	}
...
DeclarativeEnvironmentRecord.prototype.getBindingValue = function getBindingValue(n, s) {
	...
	if (s && !binding.isMutable && !binding.isInitialized) {
		throwException('ReferenceError', 'Could not get value: binding "' + n + '" has not been initialized');
	}
...
ObjectEnvironmentRecord.prototype.getBindingValue = function getBindingValue(n, s) {
	...
	if (!bindingObject.hasProperty(n)) {
		if (s) {
			throwException('ReferenceError', 'Property ' + n + ' does not exist');
		}
		return new UndefinedType();
	}
...
function createArgumentsObject(func, names, args, env, strict) {
	...
	if (mappedNames.length) {
		...
		obj.defineOwnProperty = function defineOwnProperty(p, desc, throwFlag) {
			...
			if (!allowed) {
				if (throwFlag) {
					throwException('TypeError', 'Cannot define property ' + p);
				}
				return false;
			}
...
EvalFunction.prototype.call = function call(thisVal, args) {
	...
	// Step 3
	evalCtx = createEvalContext(Runtime.getCurrentContext(), prog, RuleProcessor.isBlockStrict(prog));
	...
	if (result[0] === 'normal') {
		...
		if (result[1]) {
			if (result[1].className.match('Error$')) {
				throwException(result[1].get('name'), result[1].get('message'));
			} else {
				throwException('Error', toString(result[1]).value);
			}
		} else {
...
JSONParseFunc.prototype.call = function call(thisVal, args) {
	...
	try {
		...
		throwException('SyntaxError', e.message);
	}

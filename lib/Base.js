 ...
 *
 * @method
 * @param {module:Base.BaseType} source The value to clone
 * @param {Array[module:Base.BaseType]} sourceStack The stack of source objects leading to the original source value
 *		cloned. This is used to detect circular references and handle them accordingly
 */
exports.cloneValue = cloneValue;
function cloneValue(source, sourceStack, clonedStack) {
	var cloned;
	sourceStack.push(source);
	console.log('clone');
	switch(source.className) {
		case 'Undefined':
			cloned = cloneUndefined(source);
			break;
		case 'Null':
			cloned = cloneNull(source);
			break;
		case 'String':
			cloned = cloneString(source);
			break;
		case 'Number':
			cloned = cloneNumber(source);
			break;
		case 'Boolean':
			cloned = cloneBoolean(source);
			break;
		case 'Object':
			cloned = cloneObject(source, sourceStack, clonedStack);
			break;
		case 'Function':
			cloned = cloneFunction(source, sourceStack, clonedStack);
			break;
		case 'Array':
			cloned = cloneArray(source, sourceStack, clonedStack);
			break;
		case 'RegExp':
			cloned = cloneRegExp(source, sourceStack, clonedStack);
			break;
		case 'Date':
			cloned = cloneDate(source, sourceStack, clonedStack);
			break;
		case 'Error':
			cloned = cloneError(source, sourceStack, clonedStack);
			break;
		case 'Reference':
			cloned = cloneReference(source, sourceStack, clonedStack);
			break;
		case 'Unknown':
			cloned = cloneUnknown(source);
			break;
		default:
...
/**
 * Clones an object prototype. If the prototype is one of the built-in prototypes, it
 * is not cloned, but rather just re-instantiated for performance reasons.
 *
 * @method
 * @param {module:Base.ObjectType} source The object to clone
 */
exports.cloneObjectPrototype = cloneObjectPrototype;
function cloneObjectPrototype(sourceObject, sourceStack, clonedStack) {
	if (sourceObject instanceof NumberPrototypeType) {
		return new NumberPrototypeType();
	} else if (sourceObject instanceof BooleanPrototypeType) {
		return new BooleanPrototypeType();
	} else if (sourceObject instanceof StringPrototypeType) {
		return new StringPrototypeType();
	} else if (sourceObject instanceof ObjectPrototypeType) {
		return new ObjectPrototypeType();
	} else if (sourceObject instanceof ArrayPrototypeType) {
		return new ArrayPrototypeType();
	} else if (sourceObject instanceof FunctionPrototypeType) {
		return new FunctionPrototypeType();
	} else if (sourceObject instanceof RegExpPrototypeType) {
		return new RegExpPrototypeType();
	} else if (sourceObject instanceof DatePrototypeType) {
		return new DatePrototypeType();
	} else if (sourceObject instanceof ErrorPrototypeType) {
		return new ErrorPrototypeType(sourceObject._errorType);
	} else {
		return cloneValue(sourceObject, sourceStack, clonedStack);
	}
}

/**
 * Clones a property descriptor
 *
 * @method
 * @param {module:Base.ObjectType} source The object to clone
 */
exports.cloneDescriptor = cloneDescriptor;
function cloneDescriptor(sourceDesc, sourceStack, clonedStack) {
	var previous,
		newDesc = {
	...
	if (isDataDescriptor(sourceDesc)) {
		previous = sourceStack.indexOf(sourceDesc.value);
		newDesc.value = previous != -1 ?
			clonedStack[previous] :
			cloneValue(sourceDesc.value, sourceStack, clonedStack);
		newDesc.writable = sourceDesc.writable;
	} else {
		if (sourceDesc.get) {
			previous = sourceStack.indexOf(sourceDesc.get);
			newDesc.get = previous != -1 ?
				clonedStack[previous] :
				cloneValue(sourceDesc.get, sourceStack, clonedStack);
		}
		if (sourceDesc.set) {
			previous = sourceStack.indexOf(sourceDesc.set);
			newDesc.set = previous != -1 ?
				clonedStack[previous] :
				cloneValue(sourceDesc.set, sourceStack, clonedStack);
		}
	}
	return newDesc;
}
...
function cloneObject(source, sourceStack, clonedStack) {
	var newObject = new ObjectType(source.className),
		i, len;
	clonedStack.push(newObject);
	newObject.extensible = source.extensible;
	for (i = 0, len = source._properties.length; i < len; i++) {
		newObject._properties[i] = cloneDescriptor(source._properties[i], sourceStack, clonedStack);
	}
	if (source.objectPrototype) {
		newObject.objectPrototype = source.objectPrototype &&
			cloneObjectPrototype(source.objectPrototype, sourceStack, clonedStack);
	}
	clonedStack.pop();
	return newObject;
}
...
ObjectType.prototype.get = function get(p) {
	...
	if (desc) {
		if (isDataDescriptor(desc)) {
			...
			result = desc.get && desc.get.className !== 'Undefined' ? desc.get.callFunction(this) : new UndefinedType();
		}
	}
...
ObjectType.prototype.getProperty = function getProperty(p) {
	...
	return this.objectPrototype && type(this.objectPrototype) != 'Null' && this.objectPrototype != this ?
		this.objectPrototype.getProperty(p) : undefined;
};
...
function cloneFunction(source, sourceStack, clonedStack) {
	var newFunc,
		i, len;
	if (source instanceof FunctionType) {
		...
		newFunc = new FunctionTypeBase(
			source.get('length').value,
			source.className);
		newFunc.callFunction = source.callFunction;
		newFunc.construct = source.construct;
	}
	clonedStack.push(newFunc);
	newFunc.extensible = source.extensible;
	for (i = 0, len = source._properties.length; i < len; i++) {
		newFunc._properties[i] = cloneDescriptor(source._properties[i], sourceStack, clonedStack);
	}
	if (source.objectPrototype) {
		newFunc.objectPrototype = cloneValue(source.objectPrototype, sourceStack, clonedStack);
	}
	clonedStack.pop();
	return newFunc;
}
...
function FunctionTypeBase(length, className, dontCreatePrototype) {
 ...
 *
 * @method
 * @name module:Base.FunctionType#callFunction
...
FunctionType.prototype.callFunction = function callFunction(thisVal, args, options) {
	...
	if (!Runtime.options.invokeMethods && !(options && options.alwaysInvoke)) {
		...
		funcCtx.lexicalEnvironment._ambiguousContext = !!(options && options.isAmbiguousContext) || inAmbiguousBlock;
		...
		if (hasPrimitiveBase(v)) {
			get = function get(p) {
				...
				if (isDataDescriptor(desc)) {
					...
					return desc.get.callFunction(base);
				}
			};
		} else {
			get = base.get;
...
function putValue(v, w) {
	...
	if (isUnresolvableReference(v)) {
		...
		globalObject.put(getReferencedName(v), w, false);
...
/**
 * Clones a declarative environment. Recursively clones parent environments.
 *
 * @method
 * @name module:Base.newDeclarativeEnvironment
 * @param {module:Base.LexicalEnvironment} source The lexical environment to clone
 * @returns {{@link module:Base.LexicalEnvironment} The newly created lexical environment
 * @see ECMA-262 Spec Chapter 10.2.2.2
 */
exports.cloneDeclarativeEnvironment = cloneDeclarativeEnvironment;
function cloneDeclarativeEnvironment(source) {
	var newEnvRec = new DeclarativeEnvironmentRecord(),
		outer,
		binding;

	// Clone the bindings
	for (binding in source._bindings) {
		newEnvRec._bindings[binding] = cloneValue(source._bindings[binding]);
	}
	...
	if (source.outer) {
		if (source.outer.envRec instanceof DeclarativeEnvironmentRecord) {
			outer = cloneDeclarativeEnvironment(source.outer);
		} else {
			outer = cloneObjectEnvironment(source.outer);
		}
	}
...
/**
 * Clones an object environment. Recursively clones parent environments.
 *
 * @method
 * @name module:Base.cloneObjectEnvironment
 * @param {module:Base.LexicalEnvironment} source The lexical environment to clone
 * @returns {{@link module:Base.LexicalEnvironment} The newly created lexical environment
 * @see ECMA-262 Spec Chapter 10.2.2.2
 */
exports.cloneObjectEnvironment = cloneObjectEnvironment;
function cloneObjectEnvironment(source) {
	var newEnvRec = new DeclarativeEnvironmentRecord(),
		outer;

	// Clone the bindings
	newEnvRec._bindingObject = cloneObject(source._bindingObject);
	newEnvRec._ambiguousContext = source._ambiguousContext;
...
function createModuleContext(ast, strict, createExports, ambiguous) {
	...
	var globalObject = new ObjectType(),
		env = newObjectEnvironment(globalObject, getGlobalContext().variableEnvironment),
		configurableBindings = false,
		executionContext = new ExecutionContext(
			env,
			env,
 ...
 *
 * @method
 * @name module:Base.enterAmbiguousBlock
 */
 ...
 *
 * @method
 * @name module:Base.exitAmbiguousBlock
 */
 ...
 *
 * @method
 * @name module:Base.isAmbiguousBlock
 */
...
/**
 * Enters a skipped mode
 *
 * @method
 */
exports.enterSkippedMode = enterSkippedMode;
function enterSkippedMode() {
	var i, len;
	if (!skippedModeCounter) {
		skippedContextStack = [];
		for (i = 0, len = contextStack.length; i < len; i++) {
			skippedContextStack[i] = cloneContext(contextStack[i]);
		}
	}
	skippedModeCounter++;
}
...
/**
 * Exits a skipped block
 *
 * @method
 */
exports.exitSkippedMode = exitSkippedMode;
function exitSkippedMode() {
	skippedModeCounter--;
}

/**
 * Checks if we are skipped block
 *
 * @method
 */
exports.isSkippedMode = isSkippedMode;
function isSkippedMode() {
	return !!skippedModeCounter;
}
 ...
 *
 * @method
 * @name module:Base.enterTryCatch
 */
 ...
 *
 * @method
 * @name module:Base.getCurrentContext
 ...
 *
 * @method
 * @name module:Base.inFunctionContext
 ...
 *
 * @method
 * @name module:Base.getGlobalContext
 ...
 *
 * @method
 * @name module:Base.getModuleContext
 ...
 *
 * @method
 * @name module:Base.cloneContext
 * @param {module:Base.executionContext} source	The source context to clone
 */
exports.cloneContext = cloneContext;
function cloneContext(source) {
	var newEnvRec = source.lexicalEnvironment instanceof ObjectEnvironmentRecord ?
		cloneObjectEnvironment(source.lexicalEnvironment) :
		cloneDeclarativeEnvironment(source.lexicalEnvironment);
	return new ExecutionContext(
		newEnvRec,
		newEnvRec,
		cloneObject(source.thisBinding),
		source.strict
	);
}

Object.defineProperty(exports, 'globalObject', {
	get: function () {},
	enumerable: true,
	configurable: true
});

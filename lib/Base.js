...
function findDeclarations(ast) {
	...
	AST.walk(ast, {
		...
		'function': function() {} // Empty to prevent this node from being processed
	});
...
function createEvalContext(callingContext, code, strict, isDirectEval) {
	...
	for (i = 0, len = functions.length; i < len; i++) {
		fn = functions[i].functionName;
		fo = new FunctionType(functions[i].formalParameterList, functions[i].functionBody, executionContext.lexicalEnvironment,
			executionContext.strict || RuleProcessor.isBlockStrict(functions[i].functionBody));
		funcAlreadyDeclared = env.hasBinding(fn);
...
EvalFunction.prototype.call = function call(thisVal, args, isDirectEval, filename) {
	...
	// Step 2
	prog = AST.parseString(x.value);
	if (prog.syntaxError) {
		handleRecoverableNativeException('SyntaxError', prog.message, {
				file: Runtime.getCurrentLocation().file,
				line: prog.line,
				column: prog.col
			});
		return new UnknownType();
	}
	Runtime.setAST(prog, filename);
	AST.walk(prog, {
	'*': function(node, next) {
	...
	// Step 3
	createEvalContext(Runtime.getCurrentContext(), prog, RuleProcessor.isBlockStrict(prog[1]), isDirectEval);

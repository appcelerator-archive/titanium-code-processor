...
CodeProcessor.prototype.process = function process(projectRoot, completionCallback, errorCallback) {
	...
	this.on("processingComplete", completionCallback);
	this.on("processingError", errorCallback);
	...
	(new xml2js.Parser()).parseString(fs.readFileSync(tiappxmlpath).toString(), function (err, result) {
		...
		setTimeout(function(){
			self._fireEvent("processingComplete");
		}, 500);
	});
};
 ...
 */
CodeProcessor.prototype.on = function on(name, callback, tag) {

	// Fetch the event listener set
	var eventListeners = this._getListeners(tag);
 ...
 */
CodeProcessor.prototype._fireEvent = function _fireEvent(name, data) {
	var listeners = this._currentEventListeners[name],
		i = 0,
...
CodeProcessor.prototype._processProject = function _processProject(entryPoint) {
	...
	function processProjectHelper(listenerSet) {
		...
		this._currentEventListeners = listenerSet.listeners;
		
		// Fire the process event
		this._fireEvent("projectProcessingBegin");
...
CodeProcessor.prototype.processFile = function processFile(file) {
	...
	if (fs.existsSync(file)) {
		...
		this._fireEvent("fileProcessingBegin", {
			file: file
		});

...
function init(sourceInformation, options, plugins, ast) {
	...
	// Parse the sourceMapDir option
	if (options.hasOwnProperty('sourceMapDir')) {
		Runtime.options.sourceMapDir = options.sourceMapDir;
	}
	Runtime.log('debug', 'Setting processing option: ' + (Runtime.options.sourceMapDir ? 'the source map directory is "' +
		Runtime.options.sourceMapDir + '"' : 'no source map directory is set'));
	...
	if (sourceInformation.sourceMaps) {
...
function generateResultsPages(suppressResults, resultsDirectory, theme, callback) {
	var plugins = Runtime.plugins,
		...
		numWarnings,
		errorsAndWarnings,
		baseDirectory = Runtime.sourceInformation.sourceDir + path.sep,
		resultsToLog = '',
		data,
		paddingLength;

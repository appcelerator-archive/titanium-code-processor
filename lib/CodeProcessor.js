...
var CodeProcessor = exports.CodeProcessor = function CodeProcessorConstructor(options) {
	...
	if (options) {
		...
		this._globalsWarnings = options.globalsWarnings && (options.globalsWarnings === "true" ? true : false);
		if (this._globalsWarnings !== undefined) {
			this._log("debug", (this._globalsWarnings ? "Enabling" : "Disabling") + " globals warnings");
		}

		// Parse the minify flag
		this._minify = options.minify && (options.minify === "true" ? true : false);

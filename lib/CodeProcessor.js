 ...
 *		file in this folder.
 * @param {Array[String]} plugins A list of plugins to load. Each name must correspond with a subdirectory containing
 ...
 */
exports.process = function process(projectRoot, plugins, options, completionCallback, errorCallback) {
	...
	for (; i < len; i++) {
		try {
			pluginPath = path.resolve(path.join(__dirname, "..", "plugins", plugins[i]));
			if (fs.existsSync(pluginPath)) {
				Messaging.log("debug", "Loading plugin '" + plugins[i] + "' at " + pluginPath);
				require(pluginPath)({
					Base: Base,
					Context: Context,
					Runtime: Runtime,
			...
			Messaging.log("error", "Could not load plugin '" + plugins[i] + "': " + e + "\n");
			process.exit(1);
		}
	}
 ...
 * @param {Boolea} [createExports] Whether or not to create a <code>module.exports</code> object in the file's context
 * @returns {{@link module:Base.ObjectType} | undefined} The value of module.exports, if one was requested
 */
exports.processFile = function processFile(file, createExports) {
	...
	if (fs.existsSync(file)) {
		...
		exports = RuleProcessor.processRule(root)[1];

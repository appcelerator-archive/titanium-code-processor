...
function process(projectRoot, pluginList, options, completionCallback) {
	...
	Runtime.log("debug", "Processing tiapp.xml '" + tiappxmlpath + "'");
	(new xml2js.Parser()).parseString(fs.readFileSync(tiappxmlpath).toString(), function (err, result) {
		...
		if (err) {
			...
			setTimeout(function () {
				...
				Runtime.log("debug", "Processing app main '" + entryPoint + "'");
...
function processFile(file, createExports, useCurrentContext) {
	...
	if (fs.existsSync(file)) {
		...
		Runtime.fireEvent("fileProcessingBegin", {
			file: file
		});
		...
		Runtime.fireEvent("fileProcessingEnd", {
			file: file
		});
...
function processHelper(files, pluginList, options, completionCallback) {
	...
	Runtime.on("processingComplete", completionCallback);
	...
	processingLoop: for (i = 0, len = tags.length; i < len; i++) {
		...
		try {
			...
			Runtime.fireEvent("projectProcessingBegin");
		...
		Runtime.fireEvent("projectProcessingEnd");
	}
	
	Runtime.fireEvent("processingComplete");
	return results;
}
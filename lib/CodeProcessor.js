	...
	processingLoop: for (i = 0, len = tags.length; i < len; i++) {
		...
		try {
			for(j = 0; j < files.length; j++) {
				Runtime.setEntryPointFile(files[j]);
				results = processFile(files[j]);
		...
		while (queuedFunction = Runtime.getNextQueuedFunction()) {
			...
			Base.createGlobalContext(['toplevel', []], false);
...
function processFile(file) {
	...
	if (existsSync(file)) {
		...
		if (!root.syntaxError) {
			...
			// Create the context
			Base.createGlobalContext(root, RuleProcessor.isBlockStrict(root[1]));

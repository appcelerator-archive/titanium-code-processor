	...
	processingLoop: for (i = 0, len = tags.length; i < len; i++) {
		...
		try {
			...
			if (e.isCodeProcessorNativeException) {
				results = ['throw', Runtime.nativeException, undefined];
				Runtime.reportError('uncaughtException', 'An exception was thrown but not caught: ' + 
					Runtime.nativeException._lookupProperty('message').value.value, {
						description: 'Uncaught exception',
						exception: Runtime.nativeException
					});

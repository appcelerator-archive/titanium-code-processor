	...
	processingLoop: for (i = 0, len = tags.length; i < len; i++) {
		...
		try {
			...
			if (e.reportError) {
				...
				results = ['throw', new Base.StringType(e.message), undefined];
			} else {
				throw e;
			}
		}
...
function processFile(file, createExports) {
	...
	if (fs.existsSync(file)) {
		...
		if (root) {
			...
			strict = children[0] && children[0][0].name === 'directive' && children[0][1] === 'use strict';
			context = Base.createGlobalContext(root, strict);

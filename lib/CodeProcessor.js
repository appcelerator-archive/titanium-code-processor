...
/**
 * Checks if an event name is a process event. If not, then this is a rule event
 * 
 * @private
 * @function
 * @param {String} name The name of the event to check.
 * @returns {Boolean} Whether or not the event is a process event.
 */
function isProcessEvent(name) {
	return ~["fileInclude", "fileRequire", "fileProcessingBegin", "fileProcessingEnd", "fileLoadError", "contextChange", "processingError", "processingComplete", "ruleEvent"].indexOf(name);
}
...
var CodeProcessor = exports.CodeProcessor = function CodeProcessorConstructor(options) {
	...
	this._taggedRuleEventListeners = [];
	this._defaultRuleEventListeners = [];
};
...
CodeProcessor.prototype.process = function process(projectRoot, completionCallback, errorCallback) {
	...
	(new xml2js.Parser()).parseString(fs.readFileSync(tiappxmlpath).toString(), function (err, result) {
		...
		setTimeout(function(){
			self._fireProcessEvent("processingComplete");
		}, 1000);
	});
};
 ...
 */
CodeProcessor.prototype.addEventListener = function addEventListener(name, callback, tag) {
	var eventListeners;

	if (isProcessEvent(name)) {
		eventListeners = this._processEventListeners;
		...
		// Fetch the appropriate event listener set
		if (tag) {
			var i = 0,
				taggedRuleEventListeners = this._taggedRuleEventListeners,
				len = taggedRuleEventListeners.length;
			for(; i < len; i++) {
				if (!taggedRuleEventListeners[i].tag === tag) {
					eventListeners = taggedRuleEventListeners[i].listeners;
				}
			}
			if (!eventListeners) {
				taggedRuleEventListeners.push({
					tag: tag,
					listeners: eventListeners = []
				});
			}
		} else {
			eventListeners = this._defaultRuleEventListeners;
		}
 ...
 */
CodeProcessor.prototype._fireProcessEvent = function _fireProcessEvent(name, data) {
	var listeners = this._processEventListeners[name],
		i = 0,
 ...
 */
CodeProcessor.prototype._fireRuleEvent = function _fireRuleEvent(rule, value) {
	this._log("debug", "Rule event '" + rule[0].name + "': " + JSON.stringify(value));

	var listeners = this._currentRuleListeners[rule[0].name],
		i = 0,
		len = listeners ? listeners.length : 0
	for(; i < len; i++) {
...
CodeProcessor.prototype._processProject = function _processProject(entryPoint) {
	...
	var i = 0,
		len = this._taggedRuleEventListeners.length;
	...
	for(; i < len; i++) {
		this._currentRuleListeners = this._taggedRuleEventListeners[i].listeners;
		processProjectHelper.call(this);
	}

	// Parse the project for the default listeners
	this._currentRuleListeners = this._defaultRuleEventListeners;
	processProjectHelper.call(this);
};
 ...
 */
CodeProcessor.prototype._processFile = function _parserFile(file, createContext) {

	// Make sure the file exists
	if (path.existsSync(file)) {

		// Fire the parsing begin event
		this._fireProcessEvent("fileProcessingBegin", {
			file: file
		});
		...
		this._fireProcessEvent("fileLoadError", {
			file: file,
 ...
 * @private
 */
CodeProcessor.prototype._createScope = function _createScope(name, file, line, column) {
	var newScope = {
		context: {
			name: name,
			file: file,
...
CodeProcessor.prototype._enterScope = function _enterScope(scope) {
	...
	this._fireProcessEvent("contextChange", {
 ...
 * @private
 * @function
 * @param {DictionaryTree} tree The tree to parse.
 */
CodeProcessor.prototype._processRule = function _processRule(rule) {
	...
	this._fireRuleEvent(rule, this['_processRule' + capitalize(ruleInfo.name)](rule));
};

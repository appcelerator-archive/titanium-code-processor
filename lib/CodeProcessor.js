...
function processEntryPoint(entryPoint) {
	...
	Runtime.log('info', 'Processing entry point ' + entryPoint);
...
function processUnvisitedCode() {
	...
	while(unprocessedFile = Runtime.getUnprocessedFilesList()[0]) {
		...
		Runtime.log('debug', 'Processing file ' + unprocessedFile);
		...
		if (!ast.syntaxError) {
			...
			// Create the context
			Base.createGlobalContext(ast, RuleProcessor.isBlockStrict(ast), true);

			// Process the code
			Runtime._unknown = false;
			RuleProcessor.processRule(ast);
		...
		// Fire the parsing end event
		Runtime.fireEvent('fileProcessingEnd', 'Processing finished for file "' + unprocessedFile + '"', {
			filename: unprocessedFile
		});
	}
}

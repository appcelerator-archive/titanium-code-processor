...
function run(entryPoint, options, plugins, logger, callback) {
	...
	generateResultsPages(Runtime.options.logResults, Runtime.options.resultsPath, Runtime.options.resultsTheme, function () {
...
function init(options, plugins, ast) {
	...
	// Parse the logResults option
	if (options.hasOwnProperty('logResults')) {
		Runtime.options.logResults = !!options.logResults;
	}
	Runtime.log('debug', 'Setting processing option: results will' +
		(Runtime.options.logResults ? '' : ' not') + ' be logged');
	...
	for (i = 0, ilen = plugins.length; i < ilen; i++) {
		...
		loadedPlugins[i].name = pluginDefinitions[i].name;
		loadedPlugins[i].displayName = pluginDefinitions[i].displayName;
	}
	...
	for (i = 0, ilen = pluginDependencyOrder.length; i < ilen; i++) {
		for (j = 0, jlen = pluginDefinitions.length; j < jlen; j++) {
			if (pluginDependencyOrder[i].name === pluginDefinitions[j].name) {
				Runtime.log('debug', 'Initializing plugin ' + loadedPlugins[j].name);
...
function processEntryPoint(entryPoint) {
	...
	try {
		...
		if (results && results[0] === 'throw') {
			...
			Runtime.reportError('uncaughtException', exception);
		}
	} catch (e) {
...
function generateResultsPages(logResults, resultsDirectory, theme, callback) {
	...
	if (numErrors || numWarnings) {
		errorsAndWarnings = '';
		if (numErrors) {
			errorsAndWarnings += numErrors === 1 ? '1 error ' : numErrors + ' errors ';
			if (numWarnings) {
				errorsAndWarnings += 'and ';
			}
		}
		if (numWarnings) {
			errorsAndWarnings += numWarnings === 1 ? '1 warning ' : numWarnings + ' warnings ';
		}

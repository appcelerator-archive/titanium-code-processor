 ...
 * @param {CLI Logger} logger A logger instance from the CLI (or just pass in a winston instance);
 * @param {Object} config The configuration from the CLI
...
function process(files, pluginList, logger, cli, options) {
	...
	for (; i < len; i++) {
		...
		if (fs.existsSync(pluginPath)) {
			...
			plugins[pluginList[i]] = new (require(pluginPath))(cli);
		}
	}

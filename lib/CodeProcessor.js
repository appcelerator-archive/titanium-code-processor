...
function run(entryPoint, options, plugins, logger, callback) {
	...
	init(entryPoint, options, plugins);
 ...
 */
exports.init = init;
function init(entryPoint, options, plugins) {
	...
	Runtime.fileList = CodeProcessorUtils.findJavaScriptFiles(path.resolve(path.dirname(entryPoint)));
	Runtime.fireEvent('fileListSet', 'The list of files in the project was set', {
		fileList: Runtime.fileList
	});
	for (i = 0, ilen = Runtime.fileList.length; i < ilen; i++) {
		Runtime.fileList[i] = path.join(path.dirname(entryPoint), Runtime.fileList[i]);
	}
}

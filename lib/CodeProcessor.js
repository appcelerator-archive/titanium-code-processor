 ...
 * 		in this folder.
 * 	@param {Object} [options] Options for controlling how code is processed.
 ...
 */
var CodeProcessor = exports.CodeProcessor = function CodeProcessorConstructor(projectRoot, options) {
	...
	log("debug", "Creating CodeProcessor instance with project root '" + projectRoot + "'");
	
	// Process the options
	if (options) {
		this._globalsWarnings = options.globalsWarnings;
		if (this._globalsWarnings !== undefined) {
			log("debug", (this._globalsWarnings === "true" ? "Enabling" : "Disabling") + " globals warnings");
		}
	}

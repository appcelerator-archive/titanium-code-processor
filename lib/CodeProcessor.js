...
function init(sourceInformation, options, plugins, ast) {
	...
	// Calculate the blacklisted files
	if (options.hasOwnProperty('blacklistedFiles')) {
		Runtime.options.blacklistedFiles = options.blacklistedFiles;
	}
	Runtime.log('debug', 'Setting the files blacklist:\n' +
		options.blacklistedFiles.map(function (f) { return '\t' + f; }).join('\n'));

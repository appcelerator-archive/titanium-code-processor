...
function run(entryPoint, options, plugins, logger) {
	...
	if (options['results-dir']) {
		generateResultsPages(options['results-dir']);
	}
...
function init(options, plugins, ast) {
	...
	for (i in plugins) {
		...
		try {
			...
			Runtime.log('warning', 'Could not parse package.json for plugin "' + i + '": ' + e);
		}
		for(j in pkg.dependencies) {
			if (!(j in plugins)) {
				Runtime.log('error', 'Missing dependency: plugin "' + i + '" depends on plugin "' + j + '"\n');
				process.exit(1);
			}
		}
...
function generateResultsPages(resultsDirectory) {
	...
	fs.writeFileSync(path.join(resultsDirectory, 'style.css'), fs.readFileSync(path.join(__dirname, '..', 'templates', 'style.css')));
	...
	if (header.length > 1) {
		...
		for (name in plugins) {
			...
			if (plugin.displayName && plugin.getResultsPageData) {
				pluginList.push({
					name: plugin.displayName,
					overview: results.plugins[name].summary || ''
				});
			}
		}
	}
	...
	for (name in plugins) {
		...
		if (plugin.displayName && plugin.getResultsPageData) {
			i++;
			plugin = plugin.getResultsPageData(baseDirectory);
			queueRender(plugin.template,
				path.join(resultsDirectory, name + '.html'),
				i,
				plugin.data);
		}
	}
